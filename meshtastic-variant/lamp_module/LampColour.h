...
// ============================================================
// The whole generated scene is derived from a single 32-bit SEED.
//
// Why the seed matters. The engine randomises group sizes, hues and white
// levels on every tap - sending that resulting scene over the air would be
// expensive, so instead both lamps run the SAME deterministic generator from
// the SAME seed: four bytes on air reproduce an identical scene on both ends,
// randomness and all.
//
